window.FaqCategoryModal = function (manager) {
    ...
    this._onModalShown = async function (evt) {
        ...
        if (!categoryItem.length) {
            this._category = null;
            this.open({ title: __('Create Category'), confirmExit: true });
            return;
        }
        ...
        this.open({ title: __('Edit Category'), confirmExit: true });
    }
    ...
    this._onModalHide = function (e) {
        ...
        this.manager.modals.generic.open({
            ...
            content: __('Are you sure you want to close this without saving?'),
        }, () => {
            // Confirmed the exit
    ...
    this._onSaveClicked = async function (e) {
        ...
        for (const field in response.errors) {
            ...
            fieldElement.addClass('is-invalid').parent().find('.invalid-feedback').text(__(error));
        }
    }
    ...
    this._reset = function () {
        ...
        this._ui.title.text(this._options.title || __('Create Category'));
        this._ui.intro.text(this._options.intro || '');
        this._ui.btnSave.text(this._options.saveBtnLabel || __('Save')).prop('disabled', false);
